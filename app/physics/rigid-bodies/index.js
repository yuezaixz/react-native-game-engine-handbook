...
export default class RigidBodies extends Component {
    ...
    render() {
        ...
        const body = Matter.Bodies.rectangle(width / 2, -1000, boxSize, boxSize, {frictionAir: 0.021, restitution: 1});
        const floor = Matter.Bodies.rectangle(width / 2, height - boxSize / 2, width, boxSize, {
            isStatic: true,
            restitution: 1
        });
        const leftWall = Matter.Bodies.rectangle(2 / 1, height / 2, 2, height, {isStatic: true, restitution: 1});
        const rightWall = Matter.Bodies.rectangle(width - 2 / 1, height / 2, 2, height, {
            isStatic: true,
            restitution: 1
        });
        ...
        return (
            <GameEngine
                ...
                entities={{
                    ...
                    leftWall: {body: leftWall, size: [2, height], color: "#86E9BE", renderer: Box},
                    rightWall: {body: rightWall, size: [2, height], color: "#86E9BE", renderer: Box},
